...
def mostra_pontos(pnts):
    ...
    screen.blit(pontos,(270,8))     

def conta_lvl(lvl,pnts):
    if lvl == 1 and pnts >= 5000:
        lvl += 1
    if lvl == 2 and pnts >= 10000:
        lvl += 1
    if lvl == 3 and pnts >= 15000:
        lvl += 1
...
def mostra_lvl(lvl):
    ...
    nlevel = font1.render(str(lvl),1 ,(255,255,255))
    screen.blit(nlevel, (400,5))

def mostra_vida(vidas):
    font1 = pygame.font.Font(None, 20)
    life = font1.render(str(vidas),1 ,(255,255,255))
    screen.blit(life, (215,8))


def conta_vida(vidas,total):
    if vidas > 1:
        vidas -= 1
        return vidas
    if vidas == 1:
        vidas += 2
        totalz['pontos']=gameover(total)
        return vidas

def escreve_pontos(tot):
...
def dificulade(lvl):
    for asteroide in asteroides:
        if lvl == 2 and asteroide['deslocamento'] == 2:
            asteroide['deslocamento'] += 1
        if lvl == 3 and asteroide['deslocamento'] == 3:
            asteroide['deslocamento'] += 2
        if lvl == 4 and asteroide['deslocamento'] == 5:
            asteroide['deslocamento'] += 2
...
fundo_jogo = pygame.image.load(os.path.join(folder, "Game.jpg"))
fundo = pygame.transform.scale(fundo_jogo,(600,400))
...
while not sair:
    ...
    mostra_pontos(total)
    mostra_lvl(level)
    level = conta_lvl(level, total)
    mostra_vida(vidas)
    escreve_pontos(total)
